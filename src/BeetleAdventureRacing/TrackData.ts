...
import { GfxBlendFactor, GfxBlendMode, GfxBuffer, GfxBufferUsage, GfxCompareMode, GfxCullMode, GfxDevice, GfxFormat, GfxInputLayout, GfxInputLayoutBufferDescriptor, GfxInputState, GfxMegaStateDescriptor, GfxVertexAttributeDescriptor, GfxVertexBufferFrequency } from "../gfx/platform/GfxPlatform";
...
// Their progress will be set to the specified progress (TODO: although the game may end up moving it around)
...
interface CourseSpecialZones {
    specialResetZones: SpecialResetZone[];
    progressFixZones: ProgressFixZone[];
}

const COVENTRY_COVE_ZONES: CourseSpecialZones = {
    specialResetZones: [
...
const MOUNT_MAYHEM_ZONES: CourseSpecialZones = {
    specialResetZones: [],
    progressFixZones: [
...
const INFERNO_ISLE_ZONES: CourseSpecialZones = {
    specialResetZones: [
...
const SUNSET_SANDS_ZONES: CourseSpecialZones = {
    specialResetZones: [
...
const METRO_MADNESS_ZONES: CourseSpecialZones = {
    specialResetZones: [],
    progressFixZones: [
...
const WICKED_WOODS_ZONES: CourseSpecialZones = {
    specialResetZones: [],
    progressFixZones: [
...
export interface CourseTrackData {
    uvtt: UVTT
    specialZones: CourseSpecialZones
}

const sceneIndexToUVTTIndex: Record<number, number> = {
    0x5: 3,
    0x7: 0,
    0x9: 5,
    0x8: 2,
...
const sceneIndexToSpecialZones: Record<number, CourseSpecialZones> = {
    0x5: COVENTRY_COVE_ZONES,
    0x7: MOUNT_MAYHEM_ZONES,
    0x9: INFERNO_ISLE_ZONES,
    0x8: SUNSET_SANDS_ZONES,
    0xA: METRO_MADNESS_ZONES,
    0x6: WICKED_WOODS_ZONES
};

export function getTrackData(sceneIndex: number | null, filesystem: Filesystem): CourseTrackData | null {
    if(sceneIndex === null || sceneIndexToUVTTIndex[sceneIndex] === undefined) {
        return null;
    } else {    
        return { 
            uvtt: filesystem.getOrLoadFile(UVTT, "UVTT", sceneIndexToUVTTIndex[sceneIndex]),
            specialZones: sceneIndexToSpecialZones[sceneIndex]
        }
    }
}
...
export class TranslucentPlaneRenderer {
    ...
    public prepareToRender(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: ViewerRenderInput,
        ...
        let stateFlags: Partial<GfxMegaStateDescriptor> = {
            ...
            // So that they wont block each other
            depthWrite: false,
        };

        

        setAttachmentStateSimple(stateFlags, {
            blendMode: GfxBlendMode.ADD,
            blendSrcFactor: GfxBlendFactor.SRC_ALPHA,
            blendDstFactor: GfxBlendFactor.ONE_MINUS_SRC_ALPHA,
        });

        renderInst.setMegaStateFlags(stateFlags);
...
export class TrackDataRenderer
{
    ...
    public prepareToRender(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: ViewerRenderInput) {
        ...
        if(this.showSpecialResetZones) {
            for (let specialResetZone of this.trackData.specialZones.specialResetZones) {
        ...
        if(this.showProgressFixZones) {
            ...
            for (let progressFixZone of this.trackData.specialZones.progressFixZones) {
                let color;
