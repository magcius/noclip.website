...
export function parseDispositionCfg(buffer: ArrayBufferSlice): Map<number, Map<number, NPCInfo[]>> {
    ...
    const dispoInfoMap: Map<number, Map<number, NPCInfo[]>> = new Map<number, Map<number, NPCInfo[]>>(); //Progress->dayPeriod->info
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (keyword === "PROGRESS") {
            const pFlag: ProgressFlag = { flag: parseInt(line.split(" ")[1].split(",")[0]) * 100 + parseInt(line.split(" ")[1].split(",")[1]), dayPeriod: parseInt(line.split(" ")[1].split(",")[2].slice(0, -1)) };
            ...
            computeModelMatrixSRT(transform, 1, 1, 1, 0, parseFloat(data[6]), 0, parseFloat(data[3]), parseFloat(data[4]), parseFloat(data[5]));
            if (!dispoInfoMap.get(cFlag)!.has(cDayPeriod))
                dispoInfoMap.get(cFlag)!.set(cDayPeriod, []);
            dispoInfoMap.get(cFlag)!.get(cDayPeriod)!.push({ npcScript: parseInt(data[0]), npcFileName: npcTable[parseInt(data[1])], npcTransform: transform, npcEulerRotation: npcEulerRotation, npcScriptEntry: parseInt(data[7]), npcExtraResPath: data[8].slice(1, -1) });
        }
    }
    return dispoInfoMap;
}
...
export class CHR {
    public name: string;
    public resInfo: Map<String, BUNDLE.BundleResourceInfo> = new Map<String, BUNDLE.BundleResourceInfo>();
...
function parseCfgFile(buffer: ArrayBufferSlice): ChrCfgInfo {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (bMotionParse) {
            if (keyword === "KEY_END;\r")
                ...
                motionInfo.push({ name: data[0].split(" ")[1].split("\"")[1], frameStart: parseInt(data[1].split(" ")[1]), frameEnd: parseInt(data[2].split(" ")[1]), speedFactor: parseFloat(data[3].split(" ")[1].split(";")[0]) });
            }
            continue;
        }
        if (keyword === "MODEL") {
            if (line.split(",").length > 1) //"New" MDS, see tree info to understand this check's necessity
    ...
    return { mdsFileName: mdsFileName, imgFileName: imgFileName, motFileName: motFileName, motionInfo: motionInfo };
}
