...
class DescentBillboardProgram extends DeviceProgram {
    ...
    public static a_vup = 2;
    public static a_sxsy = 3;
...
layout (location = ${DescentBillboardProgram.a_vup}) in vec3 a_vup;
layout (location = ${DescentBillboardProgram.a_sxsy}) in vec2 a_sxsy;

out vec3 v_uvl;

void main() {
    // Rotate billboard towards camera around a_vup (cylindrical billboard)
    vec3 t_look = a_xyz - u_cameraPos;
    vec3 t_perp = t_look - abs(dot(t_look, a_vup)) * t_look;
    vec3 t_vright = normalize(cross(a_vup, t_perp));
    vec3 t_vforward = normalize(cross(t_perp, t_vright));
    vec2 t_off = a_sxsy.xy * (a_uvl.xy - 0.5);
    vec3 t_pos = a_xyz + t_vright * t_off.x + a_vup * t_off.y;
    vec4 t_xyz = UnpackMatrix(u_matView) * vec4(t_pos, 1.0);
...
function getObjectVclipRenderer(
    ...
    switch (render.type) {
        case DescentRenderType.HOSTAGE:
            return render;
        // Ignore FIREBALL, POWERUP and WEAPONVCLIP on purpose, even though they are also rendered as VClips.
    }
    return null;
}
...
export class DescentHostageRenderer {
    ...
    constructor(
        ...
        const vertexAttributeDescriptors: GfxVertexAttributeDescriptor[] = [
            ...
            {
                location: DescentBillboardProgram.a_vup,
                bufferIndex: 0,
    ...
    public prepareToRender(
        ...
        const cameraPos = viewerInput.camera.worldMatrix;
        ...
        mappedScene[offset++] = cameraPos[12];
        mappedScene[offset++] = cameraPos[13];
        mappedScene[offset++] = cameraPos[14];
