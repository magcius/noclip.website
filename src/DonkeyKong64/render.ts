...
`;
    ...
    constructor(private DP_OtherModeH: number, private DP_OtherModeL: number, private DP_Combine: RDP.CombineParams) {
        super();
        if (F3DEX.getCycleTypeFromOtherModeH(DP_OtherModeH) === F3DEX.OtherModeH_CycleType.G_CYC_2CYCLE)
    ...
    private generateAlphaTest(): string {
        ...
        const cvgXAlpha = (this.DP_OtherModeL >>> F3DEX.OtherModeL_Layout.CVG_X_ALPHA) & 0x01;
...
`;
    ...
    private generateFrag(): string {
        const textFilt = F3DEX.getTextFiltFromOtherModeH(this.DP_OtherModeH);
...
export function textureToCanvas(texture: RDP.Texture): Viewer.Texture {
    ...
    extraInfo.set('Format', F3DEX.getImageFormatString(texture.tile.fmt, texture.tile.siz));
...
function translateTexture(device: GfxDevice, texture: RDP.Texture): GfxTexture {
...
function translateSampler(device: GfxDevice, cache: GfxRenderCache, texture: RDP.Texture): GfxSampler {
...
export class RenderData {
    ...
    constructor(device: GfxDevice, cache: GfxRenderCache, public sharedOutput: F3DEX.RSPSharedOutput, dynamic = false) {
...
class DrawCallInstance {
    ...
    constructor(device: GfxDevice, cache: GfxRenderCache, geometryData: RenderData, private drawMatrix: mat4[], private drawCall: F3DEX2.DrawCall) {
    ...
    private createProgram(): void {
        const program = new F3DEX2_Program(this.drawCall.DP_OtherModeH, this.drawCall.DP_OtherModeL, this.drawCall.DP_Combine);
    ...
    public prepareToRender(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: Viewer.ViewerRenderInput, isSkybox: boolean): void {
        ...
        let offs = renderInst.allocateUniformBuffer(F3DEX2_Program.ub_DrawParams, 12 + 8*2);
        ...
        offs = renderInst.allocateUniformBuffer(F3DEX2_Program.ub_CombineParams, 9);
        ...
        comb[offs] = (this.drawCall.SP_GeometryMode & F3DEX.RSP_Geometry.G_LIGHTING) ? 1.0 : 0.0; // Lighting flag
        offs++;
