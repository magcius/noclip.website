...
void mainPS() {
...
#ifdef USE_DETAIL
    vec4 t_DetailTexture = DebugColorTexture(texture(SAMPLER_2D(u_TextureDetail, v_TexCoord0.zw)));
    ...
    vec3 t_NormalWorld;
#ifdef USE_BUMPMAP
    vec4 t_BumpmapSample = texture(SAMPLER_2D(u_TextureBumpmap, v_TexCoord2.xy));
    ...
    t_DiffuseLighting.rgb = texture(SAMPLER_2D(u_TextureLightmap), t_ProjTexCoord).rgb;
#endif
    ...
    vec3 t_EnvmapColor = texture(SAMPLER_2D(u_TextureEnvmap), t_Reflection).rgb;
...
float CalcFogAmountFromScreenPos(vec2 t_ProjTexCoord) {
    float t_DepthSample = texture(SAMPLER_2D(u_TextureDepth), t_ProjTexCoord).r;
...
uniform sampler2D u_TextureLightmap;
uniform sampler2D u_TextureNormalmap;
uniform sampler2D u_TextureFlow;
...
void mainPS() {
    ...
    vec4 t_FlowSample = texture(SAMPLER_2D(u_TextureFlow), t_FlowTexCoord.xy);
