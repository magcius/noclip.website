...
const toDataView = (bytes: Uint8Array) => new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
...
export const parse = async (scxBytes: Uint8Array): Promise<SCX.Scene> => {
    ...
    for (let i = 0; i < len; i++) {
        ...
        const isFieldName = !isValue && tokenTable[scxBytes[i + 1]]?.name !== openBrace;
        if (isValue) {
    ...
    print(): SCX.Scene {
        ...
        this.crawlObject(o, (o: Record<string, any>) => {
            ...
            if (o.keycount !== undefined && o.keyframes !== undefined) {
                const keyframes: Keyframe[] = range(0, o.keycount).map((i): Keyframe => {
                    const [time, value, tangentIn, tangentOut] = o.keyframes.slice(i * 4, (i + 1) * 4);
