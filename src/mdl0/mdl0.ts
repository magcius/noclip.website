...
export function parse(buffer:ArrayBuffer):MDL0 {
    ...
    if (flags & Flag.HAS_UV) {
        // XXX: How to parse UV?
        const start = offs, end = start + vertCount * 8;
        offs = end;
    }

    let clrData;
    if (flags & Flag.HAS_COLOR) {
        const start = offs, end = start + vertCount * 4;
    ...
    let idxData;
    {
        const start = offs + 0x02, end = start + (idxCount * 0x02);
    ...
    {
        const start = offs, end = start + animCount * animSize;
