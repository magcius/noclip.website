...
#define u_EyePos (u_Params[1].xyz)
...
void main() {
    ...
    vec3 dir = t_PositionWorld.xyz - u_EyePos;
    dir.z *= 3.0; // Flatten the sphere for dome-like appearance
    v_SkyDir = dir;
}
`;
...
void main() {
    vec2 uv = normalize(v_SkyDir).xy * (189.0 / 64.0);

    vec2 solidUV = uv + u_Time / 16.0;
    vec4 solidColor = texture(SAMPLER_2D(u_TextureSolid), solidUV);

    vec2 alphaUV = uv + u_Time / 8.0;
