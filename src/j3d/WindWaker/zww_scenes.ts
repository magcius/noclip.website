...
import { J3DModelInstanceSimple, J3DModelData, BMDModelMaterialData, MaterialInstance } from '../../Common/JSYSTEM/J3D/J3DGraphBase';
...
import { ColorKind, fillSceneParamsDataOnTemplate, GXMaterialHelperGfx } from '../../gx/gx_render';
...
class SceneDesc {
    ...
    private async spawnObjectsForActor(device: GfxDevice, renderer: WindWakerRenderer, roomRenderer: WindWakerRoomRenderer, name: string, parameters: number, layer: number, localModelMatrix: mat4, worldModelMatrix: mat4): Promise<void> {
        ...
        else if (name === 'LinkThing') {
            const patchMat = (inst: MaterialInstance) => {
                const m = inst.materialHelper.material;
                m.tevStages.length = 1;
                console.log(inst.materialHelper);
                inst.materialHelper.createProgram();
            };

            const rarc = await fetchArchive(`Link.arc`);
            const anm = await fetchArchive(`LkAnm.arc`);
            const cl = buildModel(rarc, 'bdl/cl.bdl');
            cl.modelInstance.materialInstances.filter((g) => !g.name.startsWith('eye') && !g.name.startsWith('mayu')).forEach((g) => patchMat(g));
            cl.bindANK1(BCK.parse(await Yaz0.decompress(anm.findFileData(`bcks/wait.bck`)!)));
            setToNearestFloor(cl.modelMatrix, localModelMatrix);
            cl.setMaterialColorWriteEnabled("eyeLdamA", false);
            cl.setMaterialColorWriteEnabled("eyeLdamB", false);
            cl.setMaterialColorWriteEnabled("mayuLdamA", false);
            cl.setMaterialColorWriteEnabled("mayuLdamB", false);
            cl.setMaterialColorWriteEnabled("eyeRdamA", false);
            cl.setMaterialColorWriteEnabled("eyeRdamB", false);
            cl.setMaterialColorWriteEnabled("mayuRdamA", false);
            cl.setMaterialColorWriteEnabled("mayuRdamB", false);
        }
    ...
    private spawnObjectsFromDZR(device: GfxDevice, renderer: WindWakerRenderer, roomRenderer: WindWakerRoomRenderer, buffer: ArrayBufferSlice, modelMatrix: mat4): void {
        ...
        const g = mat4.create();
        computeModelMatrixSRT(g, 1, 1, 1, 0, MathConstants.TAU * 0.4, 0, -202620, 400, 316000);
        this.spawnObjectsForActor(device, renderer, roomRenderer, 'LinkThing', 0, -1, g, modelMatrix);
    }
}
